 ...
 */
const scCalculatePriority = ([image, index, length]: [HTMLImageElement, number, number]) =>
  (image.width + ((length - index) * 10))
  * (image.classList.contains('u-photo') ? 1.2 : 1)

export const scGetImageElement = (): HTMLImageElement | null => {
  const images = [...document.images]
    ...
    .sort((a, b) => scCalculatePriority(b) - scCalculatePriority(a))
